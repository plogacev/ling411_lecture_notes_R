...
- We can translate the first row of the dataframe to English as the following: "The percentage of NP realizations when the recipient is animate is 63.4% and there are 822 rows where the recipient is animate." 
...
# When the recipient is realized as an NP, this function will put a 1 in our new column.
...
- Then, put them together in a data frame.
```{r, error=T}
...
# What is important for us is Modality, AnimacyOfTheme, LengthOfTheme, and RealizationOfRecipient.

df <- select(dative, Modality, AnimacyOfTheme, LengthOfTheme, RealizationOfRecipient)
head(df)
```

- `select()` can also *deselect* columns. Let's assume that we changed our mind and do not want to work with the Modality column. 
- Similar to the indices, minus symbol `-` deselects the specified column(s).
...
- `summarize()` is a complex function that does multiples things in a single instance. 
- Firstly, it divides a grouped data frame into subsets, with *each* subset corresponding to one value of the grouping variable (or a combination of values for several grouping variables). 
- Second, it computes one or several values we specify on each such subset.
- Lastly, it creates a new data frame and puts everything together. The first column of this new data frame consists of levels of our grouping variable. In the following columns, the `summarize()` function prints the results of the computations we have specified.
...
- In the code below, we first specify a column named `prop_NP`. Then, we use it to compute two other values named `prop_PP` and `how_many_NP_realizations`.
...
- The main difference between `mutate()` and `summarize()` is the output. While `mutate()` keeps everything from the original data frame, `summarize()` creates a brand new data frame. 
...
- Luckily, there is a better way to write these expressions.
