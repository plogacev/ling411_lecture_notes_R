...
- Let's create a simple vector with all integers from 1 to 8 and see its contents.
...
- To get our average profit, we first need the sum of the vector. 
- Then, we will divide the sum by the length of the vector.
- First, let's see the sums of these vectors.
...
- We may also need length values of these vectors:
...
- It seems that betting is more profitable. See that same division, can be done with `mean()` function.
...
- See that subtraction is applied to every element of the vector. 
...
- The best way to represent a pair of vectors where the $i$-th element in vector 1 corresponds to the $i$-th element in vector 2 is with data frames. We can introduce a new data frame with the function `data.frame()`.
```{r, error=T}
df_horse_bets <- 
...
- We can assign new columns with our assignment symbol `<-`.
- When we assign text to a specific column, R puts the specified text into every row of the column of the given data frame.
- What following code says is "Create a new column named `game` in the data frame named `df_horse_bets` and fill the column with the string _horse_bets_."
...
- `aes()` function is where we define what x and y axes are going to be. 
